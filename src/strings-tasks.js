...
function isPalindrome(str) {
  const clearStr = str.replace(/[^a-z]/gi, '').toLowerCase();
...
function extractNameFromTemplate(value) {
  ...
  return `${valueArr[1]} ${valueArr[2].replace('!', '')}`;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < strArr.length; i += 1) {
    currentIndex = inputAlph.indexOf(strArr[i]);
    if (currentIndex === -1) {
      currentIndex = inputAlph.indexOf(strArr[i + 1]);
      i += 1;
    }
    strArr.splice(i, 1, outputAlph[currentIndex]);
  }
  return strArr.join('');
}
